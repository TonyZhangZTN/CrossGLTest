...
int main()
{
    ...
    // just a simple hack to set configIndex to 0
	printf("configIndex chosen=%d\n",configIndex);
    EGLContext context =
    ...
    {
	    printf("eglMakeCurrent failed, code=%x\n",eglGetError());
    }
	...
	color_r+=polor_r*0.01f;
	color_g+=polor_g*0.01f;
	color_b+=polor_b*0.01f;
	if(color_r >1.f){
		polor_r=-polor_r;
	}
